...
| `DEFAULTS`, `LIMITS`, `IMAGE_MODEL_CAPABILITIES`, `GEMINI_PRICE_PER_IMAGE_USD`, `GEMINI_OUTPUT_SIZE_1K` (measured) | `src/constants.ts` | hand |
| Expected output sizes in both `aspect_ratio` describes and `settings.expected_size` | `expectedSize` (`src/providers/{gemini,openai}.ts` via `providers/index.ts`), rendered by `src/schemas/output-sizes.ts`; README "Output sizes" table | generated, except the README table (hand) |
| README parameter tables, model table, cost table | `README.md` | hand |
| `CLAUDE.md` gotchas and constants reference | `CLAUDE.md` | hand |
| Contract tests | `src/__tests__/server.test.ts` (enums, defaults, required, output shape, the 2,048-character cap), `src/tools/__tests__/settings.test.ts` (echoed `settings`, including `expected_size` and `matched_input_size`, equal the request sent), request-shape `toEqual` in `src/providers/__tests__/gemini.test.ts` and `openai.test.ts` | hand |
