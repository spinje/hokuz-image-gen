...
| Contract tests | `src/__tests__/server.test.ts` (enums, defaults, required, output shape, the 2,048-character cap), `src/tools/__tests__/settings.test.ts` (echoed `settings`, including `expected_size`, equal the request sent), request-shape `toEqual` in `src/providers/__tests__/gemini.test.ts` and `openai.test.ts` | hand |
