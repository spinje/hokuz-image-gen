...
2. **`num_images` is a retry loop that keeps partial results.** `runImageTool` in `src/tools/image-tool.ts` (the pipeline both handlers call) catches a per-request failure and `break`s if at least one image was collected. The warning line is the ONLY signal. Any change to that loop, the warning text, or `success` semantics must keep the shortfall visible in BOTH `content[0].text` and `structuredContent`.
3. **Two output channels that can disagree.** Every tool returns human text AND `structuredContent`. The LLM may read either. A change that updates one and not the other (count, paths, description, warning) is a silent lie in the other channel. The partial-usage scope now lives in both — the text line's "reported for n of m requests" and `usage.requests_succeeded` / `usage.requests_reported` — from the same two numbers in `runImageTool`, so a change to either channel must move the other.
...
5. **Both providers classify errors by HTTP status.** Gemini's `handleApiError` duck-types the status off the thrown error (the SDK's error classes are not exported); OpenAI's reads `status` and `code` off an `APIError`. The shared risk is a status neither switch names, which lands in the generic retryable `API_ERROR`. Two text checks remain on purpose, both inside Gemini's 400 branch: the API-key check (`/api key/i` against the message **and** the raw body, because Google returns a 400 whose reason is only in an unparsed JSON array) and the safety check (`/safety|blocked/i`, because no code identifies a block). Those two are the misfiling risk — a 400 whose message happens to say "blocked" reads as content-blocked. In both providers our own `McpError`s pass through by `instanceof`.
