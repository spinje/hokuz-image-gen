...
5. **Gemini error classification is by substring.** Gemini's `handleApiError` maps 429/"rate limit", 401/403/"api key", "blocked"/"safety" via message text. A new SDK error phrasing falls to generic `API_ERROR`; a message that happens to contain "safety" is misfiled as content-blocked. OpenAI's classifies by `status` and `code` instead, so its risk is the opposite: a status the switch does not name. In both, our own `McpError`s pass through by `instanceof`.
