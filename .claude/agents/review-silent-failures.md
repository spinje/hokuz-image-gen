...
1. **The API response is loosely typed and parsed defensively.** `parseInteraction` (`src/providers/gemini.ts`) reads `output_image`, then scans `steps[]` of type `model_output`, de-duplicates by base64 data, and falls back to `output_text`. Every branch is optional. A new response shape can produce zero usable images. The shared pipeline reports that as `API_ERROR`, preserves model text and reported usage, and must not infer moderation. Check that parsing changes do not silently discard any of those facts.
2. **Each provider response is saved before requesting another image.** A later generation or save failure must preserve saved paths and usage. The `status` and `issue` must explain the shortfall and recovery in both text and structured output.
3. **Two output channels that can disagree.** Every tool returns human text AND `structuredContent`. The LLM may read either. A change that updates one and not the other (count, paths, description, issue) is a silent lie in the other channel. The partial-usage scope now lives in both — the text line's "reported for n of m requests" and `usage.requests_completed` / `usage.requests_reported` — from the same two numbers in `runImageTool`, so a change to either channel must move the other. Likewise `settings` and the text `Settings:` line both come from one `effectiveConfig` result; `settings` must equal what the provider request was built from (provider defaults included), never the raw arguments. `images[].aspect_error_pct` and each image's text-line note must agree, and an image with no known size must never let `expected_size` read as its delivered size; `settings.expected_size` must come from `expectedSize`, never a guess for a size the provider chooses.
4. **Fallbacks that fill in a value instead of failing.** An input image's MIME comes from its extension or the `content-type` header only, and an unknown extension or a missing header is an error (`loadInputImage` in `src/services/file-utils.ts`), never a guessed `image/png`; `parseInteraction` still defaults an output image's MIME to `image/jpeg`. That extension/header MIME also decides the per-model input-type check, so a mislabelled file (a BMP named `.png`) passes our check and is refused by the provider instead. Each is a deliberate choice, but a new fallback in the same family — a default model, a default path, a default resolution — is a review point: does the caller learn that a default was substituted?
5. **Provider evidence and recovery guidance have separate homes.** Providers normalize status and explicit moderation/quota evidence; `providers/errors.ts` supplies shared recovery guidance. No response does not mean the request was never received. No image does not establish moderation. Neither provider automatically retries generation.
...
For any change to `parseInteraction` or `InteractionLike`: which response shape would now yield zero images, and what error does the user see for it? Are model text and usage preserved, and is any explanation supported by provider evidence?
