...
| `image_paths[]` (local) | `readImageAsBase64` → `fs.readFile` | any readable path, `~` expanded; size checked AFTER the full read, against the `maxBytes` the edit tool passes from the model's `maxInputImageBytes` (7 MB Gemini, 50 MB OpenAI); MIME from extension, default `image/png`, then checked against the model's `inputMimeTypes` |
| `image_paths[]` (URL) | `fetchImageAsBase64` → global `fetch` | `http:`/`https:` only (`isUrl`); **no host restriction** (localhost, link-local, private ranges all allowed); **body fully buffered before the size check** (now up to the model's 50 MB on OpenAI); no timeout; MIME from `content-type`, default `image/png`, then checked against the model's `inputMimeTypes` |
