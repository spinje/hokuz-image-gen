...
| `OPENAI_API_KEY` | `getApiKey` (`providers/openai.ts`) → `OpenAI` | never printed; the mapped errors quote `error.message` from the API body and attach the raw `APIError` as `ToolError.cause` (not serialised into the tool result) — any change that surfaces `cause` to the caller is a finding |
