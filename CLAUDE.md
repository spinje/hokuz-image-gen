...
Results are `{ status: "complete" | "partial" | "failed", images, issue?, description?, usage? }`. `images` always lists successfully saved files; partial/failed results carry an issue. `isError` is true only for failed delivery. SDK schema-validation errors remain standard MCP errors outside this envelope.
...
4. If the model belongs to a new provider, add it to `Provider`, write `providers/<name>.ts` exporting `label`, `hasApiKey`, `getApiKey`, `generateImage` and `editImage`, add it to the `PROVIDERS` table in `providers/index.ts`, and add the key to `ENV_VARS`.
