...
- `providers/__tests__/gemini.test.ts` — exact Interactions request shape, `aspect_ratio` omitted for `auto`, `parseInteraction` de-dup/fallbacks, the pixel size read from a hand-built JPEG (baseline and progressive) and none from other bytes, the per-image cost for the resolution the request was built with (reported with `costBasis: "per_image"` even when the response carries no token counts, dropped only when no price is known), API error mapping including rejection versus uncertain completion, `GEMINI_API_KEY` precedence.
- `providers/__tests__/openai.test.ts` — exact `images.generate` and `images.edit` request shapes, size derivation (spot checks plus an invariant sweep over every ratio x resolution), edit files in order with an explicit MIME type, parse of dimensions and the cost arithmetic (with `costBasis: "tokens"`), refusal of a format we did not request, error mapping by status and by `code` including rejection versus uncertain completion, missing-key path and the client's `logLevel`.
- `providers/__tests__/index.test.ts` — dispatch to the provider the registry names, and validation before dispatch.
- `tools/__tests__/image-tool.test.ts` — the pipeline both tools share, driven once through `hokuz_generate_image`: the `num_images` loop and its `-N` filenames, a single response carrying more images than asked for, partial delivery and recovery advice, summed `usage` — the scope in **both** channels as `requests_completed`/`requests_reported` and the text line's "reported for n of m requests", counts summed over the reports that carry them and omitted rather than zeroed when none does — and a provider that reports neither usage nor dimensions.
- `tools/__tests__/settings.test.ts` — the `settings` echo equals what the provider was sent: it mocks the Google and OpenAI SDKs (not `providers/index.js`) so one call drives both the echo and the real request builder. Gemini defaults, OpenAI's quality default and background, edit `auto` on each provider, the text line, and no `settings` before generation.
...
- `__tests__/server.test.ts` — tool names, annotations, JSON Schema enums/defaults/`required`, output schema shape (including `settings`), package.json version, both descriptions and the server instructions within Claude Code's 2,048-character cap (gotcha 8), and the drift guard: every `IMAGE_MODELS` ID and every `QUALITIES` value is named in both `TOOL_DESCRIPTION`s, and every ID in both `model` describe strings. `__tests__/constants.test.ts` — validation messages and the "defaults are valid for every model" invariant.
...
Results are `{ status: "complete" | "partial" | "failed", images, settings?, issue?, description?, usage? }`. `settings` is the provider's `effectiveConfig` (defaults applied), present whenever the pipeline ran and absent for rejections before it; the text channel repeats it on a `Settings:` line. Its `aspect_ratio` is the requested target; `images[].width/height` are what was delivered. `images` always lists successfully saved files; partial/failed results carry an issue. `isError` is true only for failed delivery. SDK schema-validation errors remain standard MCP errors outside this envelope.
...
1. Add the exact ID to `IMAGE_MODELS` and an entry to `IMAGE_MODEL_CAPABILITIES`. A Gemini model also needs a row in `GEMINI_PRICE_PER_IMAGE_USD` with a price for each of its resolutions, or `constants.test.ts` fails and its results would carry no cost.
...
4. If the model belongs to a new provider, add it to `Provider`, write `providers/<name>.ts` exporting `label`, `hasApiKey`, `getApiKey`, `effectiveConfig`, `generateImage` and `editImage`, add it to the `PROVIDERS` table in `providers/index.ts`, and add the key to `ENV_VARS`.
