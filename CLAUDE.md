...
**Response parsing** (`parseInteraction`, exported): reads `interaction.output_image` first, then scans `steps[]` of type `model_output` for additional `image`/`text` blocks (de-duplicated by data), falls back to `output_text` for the description, and throws `CONTENT_BLOCKED` if no image was found. Each image's `width`/`height` come from a module-private `jpegDimensions` that walks the JPEG's segments to its SOF header — the API reports no size, and these models return JPEG only. The **price decides whether there is a `usage` report at all**: `estimatedCostUsd = imagePriceUsd × images.length` from `GEMINI_PRICE_PER_IMAGE_USD[model][resolution]` for the resolution the request was built with, with `costBasis: "per_image"`. `total_input_tokens` / `total_output_tokens` are attached only when the response carries them, because they did not produce that cost (Google bills per image: 0.5K and 1K report the same 1120 output image tokens). A model/resolution with no price yields no `usage` rather than a wrong one — the image is still returned.

**Error mapping** (`handleApiError(error, model)`): `McpError`s we raised pass through unchanged; everything else is classified by HTTP `status`, like OpenAI's. It **duck-types**: the SDK rejects with an internal Stainless-style hierarchy (`BadRequestError`, `NotFoundError`, ...) that `@google/genai` does not export — the exported `ApiError` is a different class and is not what is thrown — so there is nothing to `instanceof`. No `status` at all means the request never reached Google (connection/timeout/abort) → a `(network)` `API_ERROR`; 401/403 → `MISSING_API_KEY`; 404 → `API_ERROR` naming the model (a 404 is also how an unoffered resolution comes back, not only a retired ID); 429 → `API_RATE_LIMIT`; other 4xx → `API_ERROR` marked unretryable, except 408/409 which fall through; 5xx → a retryable `API_ERROR`.

Two text checks survive inside the 400 branch, deliberately. A rejected API key comes back as a **400** with nothing to distinguish it by code, and its message is not in the parsed `error` at all — Google returns an unparsed body that is a JSON **array** — which is why the key check also reads `error.body`. A safety block likewise carries no code (no real block was triggered while capturing these shapes). `apiMessage(error)` is where Google's own message comes from: `error.error.error.message`, else the body parsed as JSON (element 0 if it is an array), else the SDK's `message` with its leading `"404 "`-style status prefix stripped so our own text does not repeat it.

### OpenAI (`providers/openai.ts`)

One singleton `OpenAI` client from `OPENAI_API_KEY`. Both calls send `{ model, prompt, n: 1, size, quality, output_format, background }`, where `background` is `"transparent"` only when the caller asked and `"opaque"` otherwise (never `"auto"`: the cost and the alpha channel must be predictable); edit adds `image`, an array of `toFile(Buffer.from(data, "base64"), "image-N.<ext>", { type: mimeType })` in input order. **The explicit `{ type }` is load-bearing**: a buffer without it uploads as `application/octet-stream` and the API rejects it. There is no `input_fidelity` — the SDK's doc comment claims it exists, but every current GPT Image model rejects it (`invalid_input_fidelity_model`), so it is not a parameter. The Images API has no `temperature`. The client is built with `logLevel: "warn"` so an ambient `OPENAI_LOG=debug` cannot write to stdout (gotcha 4). The SDK's default retries are kept — up to two, on 429, 5xx and connection errors, honouring `retry-after` — because a transient failure is worth retrying transparently; the `num_images` loop counts our requests, not HTTP attempts, so a retried request is still one image.
...
All failures become `McpError(type, message, details?, { retryable }?)` with an actionable message that starts with `Error:`. Handlers catch everything and return `{ content: [{ type: "text", text }], structuredContent: { success: false, images: [], error, error_type, retryable }, isError: true }`, where `error_type` is the `McpError`'s own `type` (`UNKNOWN_ERROR` for anything else) so the caller can choose its next step without parsing the message; `schemas/output.ts` publishes the enum and its describe string says what each value means. `retryable` answers what `error_type` cannot, because `API_ERROR` spans both a 500 and a 400 the API will reject again: it is the `McpError`'s own `retryable` when it has one, else `RETRYABLE_BY_TYPE` in `tools/image-tool.ts` (`API_RATE_LIMIT`, `API_ERROR` and `SERVER_BUSY` true, every other type false), and `false` for anything that is not an `McpError`. It is an options object rather than a fourth positional argument so a bare `new McpError(type, message, false)` cannot land on `details` and leave the verdict unset. Two places set it, both where one type covers two different failures: the provider mappers, on the branches where the HTTP status is the evidence (their 5xx, 408/409 and no-status tails leave it unset so the table's "retry" stands), and `loadInputImage`'s remote fetch, where `INVALID_IMAGE_PATH` covers both a wrong URL and a timeout or a failing host. Validation (`validateGenerationConfig`, including the input-image count) runs before any API call and, in edit, before any image is loaded; each image's type and size are then checked by `loadInputImage` before its bytes are read.
...
`num_images > 1` is **repeated independent requests**, not an API count parameter (OpenAI's `n` is billed the same way and would return one aggregate `usage`). The loop stops early on a failure if at least one image was collected; the shortfall and the failing request's message go into `warning`, in both the response text and `structuredContent`. `usage` is summed across the iterations. Edit `aspect_ratio: "auto"` means `config.aspectRatio` is `undefined`; never coerce it to `1:1`. Gemini omits `response_format.aspect_ratio` and still applies resolution. OpenAI sends `size: "auto"` and rejects explicit resolution. Generative edits on either provider can change composition and details; auto does not guarantee original framing or pixel-identical preservation.
...
Pass the MCP request abort signal separately from GenerationConfig through loaders/providers and into the shared pipeline. Cancellation stops further requests/previews and maps to non-retryable REQUEST_CANCELLED. Save already-returned paid results even after cancellation. In-flight native work retains the active slot until it settles; a JavaScript cancellation is not native-process termination.
