...
- `tools/__tests__/settings.test.ts` — the `settings` echo equals what the provider was sent: it mocks the Google and OpenAI SDKs (not `providers/index.js`) so one call drives both the echo and the real request builder. Gemini defaults, OpenAI's quality default and background, `expected_size` equal to the `size` sent (OpenAI) and to the measured table (Gemini, including 2K and none at 0.5K), edit `auto` on each provider (no `expected_size`, no `aspect_error_pct`), the text line, and no `settings` before generation.
