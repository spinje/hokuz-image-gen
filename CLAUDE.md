...
- `services/__tests__/file-utils.test.ts` — output path rules (trailing separator, extension replacement per output format, `-N` suffixes in both modes, `~`), and `loadInputImage`'s check order: unknown extension, an unaccepted type and an oversize file all rejected with `readFile` never called (it is spied through a `vi.mock` of `fs/promises`), content-type normalisation, missing content-type, and the two size gates on a fetched body (`content-length`, then the running count while it streams).
