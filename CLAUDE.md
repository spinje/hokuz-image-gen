...
- `providers/__tests__/gemini.test.ts` — exact Interactions request shape, `aspect_ratio` omitted for `auto`, `parseInteraction` de-dup/fallbacks, the pixel size read from a hand-built JPEG (baseline and progressive) and none from other bytes, the token counts and the per-image cost for the resolution the request was built with, API error mapping, `GEMINI_API_KEY` precedence.
...
**Response parsing** (`parseInteraction`, exported): reads `interaction.output_image` first, then scans `steps[]` of type `model_output` for additional `image`/`text` blocks (de-duplicated by data), falls back to `output_text` for the description, and throws `CONTENT_BLOCKED` if no image was found. Each image's `width`/`height` come from a module-private `jpegDimensions` that walks the JPEG's segments to its SOF header — the API reports no size, and these models return JPEG only. `usage` is `{ total_input_tokens, total_output_tokens }` plus `estimatedCostUsd = imagePriceUsd × images.length`, where the price is `GEMINI_PRICE_PER_IMAGE_USD[model][resolution]` for the resolution the request was built with (Google bills per image, and the token counts cannot price one: 0.5K and 1K report the same 1120 output image tokens). A response without both totals, or a model/resolution with no price, yields no `usage` rather than a wrong one — the image is still returned.
